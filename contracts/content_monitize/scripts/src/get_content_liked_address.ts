...
async function get_content_liked_address() {
    const { keypair, client } = getExecStuff();
    const tx = new TransactionBlock();
    bcs.registerStructType('VecSet<address>', {
    keys: 'vector<address>',
    });

    tx.moveCall({
        target: `${packageId}::cep::get_content_liked_address`,
        arguments: [
    ...
    let values = bcs.de('VecSet<address>', Uint8Array.from(results.results![0].returnValues![0][0]));

    // console.log(`keys : ${inspect(keys, false, null, true)}\n`)
    console.log(`values: ${inspect(values, false, null, true)}\n`);
}
get_content_liked_address()